...
How many steps can we take at each level?
ANS= maxCol ( calculated iteratively using simple maths)
...
string convert(string chk, int numRows) {
  ...
  //calculation for maxCol ie maximum columns of all level
  int nr=2*(numRows-1);                            
	int offset=nr;         
	int maxCol=0;
	int matchSize=0;
	...
	if(matchSize>chk.size())
		if(j%2)
			maxCol=maxCol-(matchSize-chk.size());
